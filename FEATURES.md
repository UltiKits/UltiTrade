...
- **Language:** every chat line, window title, item name and lore line, BossBar title, placeholder
  ...
  `messages.*` settings of `config/trade.yml` are operator-editable: a blank value, the default since
  this change, shows the language file's text, and any other value is shown as written
  (`ultitrade.lifecycle.legacy-text-defaults`). Quotations below are `lang/en.yml`'s text with its
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultitrade.lifecycle.legacy-text-defaults | When the module is enabled and again on every reload of it, after the framework has read `config/trade.yml` and before anything reads the texts: each of `gui-title` and the seven `messages.*` values that is exactly the Chinese default an earlier version shipped is replaced with a blank value and the file is saved, so those texts come from the language file in the server's language; a value that differs in any way (an operator's edit) is kept, and a blank value is never rewritten again, so the file is saved only when something changed. A failed save is logged as one warning from the language file (`log_config_default_save_failed`) and costs nothing else (UltiKits/UltiTrade#16) | event | module enable and every reload of it: bare `/ul reload`, which reloads every module, or `/ul reload UltiTrade` | n/a | n/a | admin | brief | UltiTrade#registerSelf, UltiTrade#onReload, TradeConfig#migrateLegacyDefaults |
...
| ultitrade.config.trade.gui-title | The `TradeGUI` inventory title template, with a `{PLAYER}` placeholder for the other participant's name. Blank shows the language file's `gui_title`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: gui-title (default: blank — the language file's gui_title, under language: en "Trade with {PLAYER}")` | n/a | n/a | admin | brief | TradeGUI#TradeGUI |
...
| ultitrade.config.trade.messages.player-blocked | Message shown to a sender whose target has blocked them. Blank shows the language file's `player_blocked`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.player-blocked (default: blank — the language file's player_blocked, under language: en "You are blocked by {PLAYER}!")` | n/a | n/a | admin | brief | TradeService#sendRequest |
| ultitrade.config.trade.messages.request-received | Message shown to the recipient of a trade request, only when `enable-clickable-buttons` is false. Blank shows the language file's `request_received`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-received (default: blank — the language file's request_received, under language: en "{PLAYER} wants to trade with you! Use /trade accept or /trade deny")` | n/a | n/a | admin | brief | TradeService#notifyTradeRequest |
| ultitrade.config.trade.messages.request-sent | Message shown to the sender immediately after a trade request is sent. Blank shows the language file's `request_sent`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-sent (default: blank — the language file's request_sent, under language: en "Trading request sent to {PLAYER}!")` | n/a | n/a | admin | brief | TradeService#sendRequest |
| ultitrade.config.trade.messages.request-timeout | Message shown to the recipient when their pending request expires unanswered. Blank shows the language file's `request_timeout`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-timeout (default: blank — the language file's request_timeout, under language: en "Trading request has timed out!")` | n/a | n/a | admin | brief | TradeService#cleanupExpiredRequests |
| ultitrade.config.trade.messages.trade-cancelled | Message shown to both sides when a trade is cancelled, with an optional reason appended in parentheses. Blank shows the language file's `trade_cancelled`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-cancelled (default: blank — the language file's trade_cancelled, under language: en "Trade cancelled!")` | n/a | n/a | admin | brief | TradeService#cancelTrade |
| ultitrade.config.trade.messages.trade-complete | Message shown to both sides on a successfully completed trade. Blank shows the language file's `trade_complete`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-complete (default: blank — the language file's trade_complete, under language: en "Trade completed!")` | n/a | n/a | admin | brief | TradeService#completeTrade |
| ultitrade.config.trade.messages.trade-disabled | Message shown to a sender whose target currently has trading toggled off. Blank shows the language file's `trade_disabled_target`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-disabled (default: blank — the language file's trade_disabled_target, under language: en "{PLAYER} has trading disabled!")` | n/a | n/a | admin | brief | TradeService#sendRequest |
