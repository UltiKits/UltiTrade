...
- **No manual prose:** no troubleshooting column, no explanatory paragraphs, no draft page text.
  ...
  it.
- **Language:** every chat line, window title, item name and lore line, BossBar title, placeholder
  value and console line this module writes comes from its language file (`lang/en.yml`,
  `lang/zh.yml`), so it follows the framework's `language` setting (`UltiKits/UltiTrade#16`). Before
  that change only the six `/trade toggle`, `/trade block` and `/trade unblock` replies were read from
  the language file (`UltiKits/UltiTrade#17`); everything else was fixed Chinese text, and the enable
  and disable lines passed a Chinese sentence as the key. The trade-window title and the seven
  `messages.*` settings of `config/trade.yml` are written into the file in the server's language while
  they are still built-in text, and the module sends exactly what the file holds; an operator's edit is
  kept (`ultitrade.lifecycle.legacy-text-defaults`). Quotations below are `lang/en.yml`'s text with its
  colour codes left out.
...
description = "command_description")` (a language key, "Player trading system" under
`language: en`), `@CmdTarget(PLAYER)`. A trade request is a one-to-one offer between two online
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultitrade.settings.block | Add a named, currently-ONLINE player to the sender's own trade blocklist, refusing to add self or a name that is already blocked. The success reply and the already-blocked refusal are the language catalogue's `block_success` and `already_blocked` entries, so they follow `language`; the second line after a success (`block_success_hint`) and the self and offline refusals (`cannot_block_self`, `block_player_offline`) come from the language file too (`UltiKits/UltiTrade#17`, `#16`) | command | `/trade block <player>` | ultitrade.use | player | player | brief | TradeCommand#blockPlayer, TradeLogService#blockPlayer |
...
| ultitrade.placeholder.last-trade-ago | Human-readable elapsed time since the target's last completed trade (days/hours/minutes/"just now"), from the language file (`placeholder_days_ago`, `placeholder_hours_ago`, `placeholder_minutes_ago`, `placeholder_just_now`), so it follows `language`; a target who has never traded returns `placeholder_never` ("Never") (`UltiKits/UltiTrade#16`) | placeholder | `%ultitrade_last_trade_ago%` | n/a | n/a | player | brief | TradePlaceholderExpansion#formatTimeAgo |
| ultitrade.placeholder.last-trade-time | The target's last completed-trade timestamp, formatted `yyyy-MM-dd HH:mm`; a target who has never traded returns the language file's `placeholder_never_traded` ("Never traded") (`UltiKits/UltiTrade#16`) | placeholder | `%ultitrade_last_trade_time%` | n/a | n/a | player | brief | TradePlaceholderExpansion#formatTimestamp |
...
| ultitrade.placeholder.trade-enabled-display | Localized rendering (the language file's `placeholder_enabled` / `placeholder_disabled`, "Enabled" / "Disabled" under `language: en`, `UltiKits/UltiTrade#16`) of the same toggle state as `.trade-enabled`; two aliases (`trade_enabled_display`, `enabled_display`) return the identical value | placeholder | `%ultitrade_trade_enabled_display%` or `%ultitrade_enabled_display%` | n/a | n/a | player | brief | TradePlaceholderExpansion#onRequest |
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultitrade.lifecycle.legacy-text-defaults | When the module is enabled and again on every reload of it (`UltiTrade#registerSelf`, before any service reads the texts, and `#onReload`, both after the framework has read `config/trade.yml` and loaded the language — never from a configuration change listener, which the framework fires before it reloads the language): each of `gui-title` and the seven `messages.*` values that is still built-in text — the Chinese default an earlier version shipped, or this jar's English or Chinese text for it (read from the module jar, never from the language files on disk) — and differs from the current text is replaced with the language file's text in the server's language, and the file is saved once. An untouched value therefore follows a `language` switch in both directions; a value that differs in any way, even by one character, is the operator's and is kept byte for byte; a text that would break the setting's `@NotEmpty` is never written; a second start with the same language writes nothing. A single-module `/ul reload UltiTrade` does not re-read the framework's `language`, so a changed `language` is picked up on a bare `/ul reload` or a restart. A failed save is logged as one warning from the language file (`log_config_default_save_failed`) and costs nothing else (UltiKits/UltiTrade#16) The text written is this jar's own built-in text for the server's language (the jar's `lang/<language>.*`), not the extracted language file on disk, so every value the module writes is one it recognises again; these settings are edited in the config file, and an edit of the extracted language file does not change them (earlier versions never read them from the language file either; orchestrator ruling O3, 2026-09-25). | event | module enable and every reload of it: bare `/ul reload`, which reloads every module, or `/ul reload UltiTrade` | n/a | n/a | admin | brief | UltiTrade#registerSelf, UltiTrade#onReload, TradeConfig#materializeText, ConfigTextDefaults |
...
| ultitrade.persistence.pending-stake-return | When a trade is cancelled while the server cannot find one participant (`Bukkit#getPlayer` answers nothing for them), that participant's staked items are saved in `trade_pending_returns`, one row per cancelled trade, stored with Bukkit's own item serialization so every part of each item is kept, and a console WARN names the player and the items; at that player's next join each saved row is read, removed, and only then handed over through the same give-or-drop step as a cancel (what does not fit is dropped at the player's feet), the player is told `Items you staked in a trade that was cancelled while you were away have been returned to you.`, and a console INFO line counts the stacks. A row that cannot be removed is not handed over and is tried again at the next join; a row that cannot be read is left in the table and reported. If the row cannot be saved, the items are dropped at the player's last known location (or the first world's spawn when the platform keeps none) and a console ERROR names the player and the items. Money and experience are never taken before a trade completes, so a cancel has none to keep (`UltiKits/UltiTrade#32`; maintainer decision of 2026-09-24) | persistence | a trade cancelled by the completion check, a server shutdown, a quit or `/trade cancel` while the other participant cannot be found; then that participant joins | n/a | player | player | brief | TradeService#cancelTrade(TradeSession,String), TradeService#deliverPendingReturns, TradeListener#onPlayerJoin |
...
| ultitrade.config.trade.gui-title | The `TradeGUI` inventory title template, with a `{PLAYER}` placeholder for the other participant's name. The value shown is the file's value, as written. It is written in the server's language — the language file's `gui_title` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: gui-title (default: written in the server's language; under language: en '&6Trade with {PLAYER}')` | n/a | n/a | admin | brief | TradeGUI#TradeGUI |
...
| ultitrade.config.trade.messages.player-blocked | Message shown to a sender whose target has blocked them. The value shown is the file's value, as written. It is written in the server's language — the language file's `message_player_blocked` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.player-blocked (default: written in the server's language; under language: en '&cThat player has blocked you!')` | n/a | n/a | admin | brief | TradeService#sendRequest |
| ultitrade.config.trade.messages.request-received | Message shown to the recipient of a trade request, only when `enable-clickable-buttons` is false. The value shown is the file's value, as written. It is written in the server's language — the language file's `message_request_received` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-received (default: written in the server's language; under language: en '&e{PLAYER} &fwants to trade with you! Type /trade accept to accept')` | n/a | n/a | admin | brief | TradeService#notifyTradeRequest |
| ultitrade.config.trade.messages.request-sent | Message shown to the sender immediately after a trade request is sent. The value shown is the file's value, as written. It is written in the server's language — the language file's `message_request_sent` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-sent (default: written in the server's language; under language: en '&aTrade request sent to &f{PLAYER}&a!')` | n/a | n/a | admin | brief | TradeService#sendRequest |
| ultitrade.config.trade.messages.request-timeout | Message shown to the recipient when their pending request expires unanswered. The value shown is the file's value, as written. It is written in the server's language — the language file's `request_timeout` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.request-timeout (default: written in the server's language; under language: en '&cTrading request has timed out!')` | n/a | n/a | admin | brief | TradeService#cleanupExpiredRequests |
| ultitrade.config.trade.messages.trade-cancelled | Message shown to both sides when a trade is cancelled, with an optional reason appended in parentheses. The value shown is the file's value, as written. It is written in the server's language — the language file's `trade_cancelled` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-cancelled (default: written in the server's language; under language: en '&cTrade cancelled!')` | n/a | n/a | admin | brief | TradeService#cancelTrade |
| ultitrade.config.trade.messages.trade-complete | Message shown to both sides on a successfully completed trade. The value shown is the file's value, as written. It is written in the server's language — the language file's `trade_complete` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-complete (default: written in the server's language; under language: en '&aTrade completed!')` | n/a | n/a | admin | brief | TradeService#completeTrade |
| ultitrade.config.trade.messages.trade-disabled | Message shown to a sender whose target currently has trading toggled off. The value shown is the file's value, as written. It is written in the server's language — the language file's `message_trade_disabled` — while it is still built-in text, so an untouched value follows `language` (`ultitrade.lifecycle.legacy-text-defaults`, UltiKits/UltiTrade#16) | config | `config/trade.yml: messages.trade-disabled (default: written in the server's language; under language: en '&cThat player has trading disabled!')` | n/a | n/a | admin | brief | TradeService#sendRequest |
...
Two JUnit guards (`UltiTradeLanguageCatalogueTest`, `UltiTradeCjkLiteralScopeTest`) fail the build
