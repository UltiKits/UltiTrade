...
| ultitrade.session.confirm-large | For a trade whose total money OR total experience meets `confirm-threshold`, clicking Confirm the first time opens `TradeConfirmPage` instead of confirming immediately; confirming there marks the confirmation and returns to `TradeGUI` | event | click the Confirm slot in `TradeGUI` when the trade's total money or experience is at least `confirm-threshold` | n/a | n/a | player | detailed | TradeService#confirmTrade, TradeConfirmPage#handleClick |
...
| ultitrade.session.item-remove | Remove a previously placed item from one of the sender's own item slots back into the sender's own inventory, resetting both sides' confirmation | event | click a "your items" slot in `TradeGUI` that already holds a placed item | n/a | n/a | player | brief | TradeListener#onInventoryClick |
...
| ultitrade.session.confirm-page | The second-step confirmation window for a large trade (5-row raw `InventoryHolder`): an info item summarizing what each side gives/receives after tax, up to 3 item-preview slots per side (a 4th "N more items..." indicator beyond that), and Confirm/Cancel buttons; Cancel returns to `TradeGUI` WITHOUT cancelling the trade itself | gui | opened by `ultitrade.session.confirm-large` | n/a | n/a | player | brief | TradeConfirmPage#TradeConfirmPage, TradeConfirmPage#initializeGUI |
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultitrade.config.trade.allow-cross-world | Whether a trade request may be sent across two different worlds | config | `config/trade.yml: allow-cross-world (default: false)` | n/a | n/a | admin | brief | TradeService#sendRequest |
